...
The 'agecov' mechanism is limited to sampling sessions separated by one time unit. A more flexible method is provided from 2.2.6: specify the details argument `ageclassfn`, a simple function to group numeric ages into age classes. Test your function by applying it to the matrix of numeric ages in your data. For example,
```{r ageclass}
# a possible ageclassfn (should return a factor)
agefn <- function(age) cut(age, breaks = c(0,1.99,Inf), 
  labels = c('young', 'old'), include.lowest = TRUE)
# construct matrix of numeric ages (animal x secondary session)
age <- age.matrix(join(ovenCH), maximumage = 6)
